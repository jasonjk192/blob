...
public class generateGroundScript : MonoBehaviour
{
    ...
    static public int probDensity = 30;
    ...
    void GenerateEnvironmentObjects()
    {
        ...
        for (int x = 1; x < ground_size - 1; x += space)
        {
            ...
            {
                var rockPrefab = Instantiate(rocks[Random.Range(0, rocks.Length - 1)], new Vector3(x - hs, 0, offset - scaleFactor + hs), Quaternion.identity);
            ...
            for (int i = 0; i < space; i++)
            {
                scaleFactor = Random.Range(1f, 5f);
                var rockPrefab = Instantiate(rocks[Random.Range(0, rocks.Length - 1)], new Vector3(x - hs, 0, offset + scaleFactor - hs), Quaternion.identity);
                rockPrefab.transform.localScale = new Vector3(scaleFactor, scaleFactor, scaleFactor);
                rockPrefab.tag = "EnvironmentObjects";
                CreateChildPrefab(rockPrefab, rocksParent);
            ...
            {
                scaleFactor = Random.Range(1f, 5f);
                var rockPrefab = Instantiate(rocks[Random.Range(0, rocks.Length - 1)], new Vector3(offset - scaleFactor + hs, 0, x - hs), Quaternion.identity);
                rockPrefab.transform.localScale = new Vector3(scaleFactor, scaleFactor, scaleFactor);
                rockPrefab.tag = "EnvironmentObjects";
                CreateChildPrefab(rockPrefab, rocksParent);
            }
            ...
            if (x <= space || x >= ground_size - space)
                continue;*/
