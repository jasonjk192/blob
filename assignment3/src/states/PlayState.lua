...
local helpTileY, helpTileX, helpTile2X, helpTile2Y = -1,-1, -1,-1
local highlightHelpTile = false
...
function PlayState:enter(params)
    ...
    self.board = params.board or Board(VIRTUAL_WIDTH - 272, 16, level)
    ...
    helpTileY,helpTileX,helpTile2Y, helpTile2X=self.board:checkPossibleMoves()
    while(helpTileY==-1) do
...
function PlayState:calculateMatches()
    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            for km, t in pairs(match) do
                self.score = self.score + t.variety*10
            end
...
function PlayState:render()
    ...
    love.graphics.setColor(50/255, 50/255, 172/255, 1)
    love.graphics.setLineWidth(2)
    love.graphics.rectangle('line', (helpTileX-1) * 32 + (VIRTUAL_WIDTH - 272),
            (helpTileY-1) * 32 + 16, 32, 32, 4)
    love.graphics.setLineWidth(2)
    love.graphics.rectangle('line', (helpTile2X-1) * 32 + (VIRTUAL_WIDTH - 272),
            (helpTile2Y-1) * 32 + 16, 32, 32, 4)
    ...
    displayText('Tile 1 : '..tostring(helpTileX)..', '..tostring(helpTileY) , 5, 5)
    displayText('Tile 2 : '..tostring(helpTile2X)..', '..tostring(helpTile2Y) , 5, 15)
end
