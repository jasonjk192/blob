...
function Level:update(dt)
    ...
    if self.collisionOccured then
        self.launchMarker.allowTripleBird = false
    end
    self.launchMarker:update(dt)

    if self.launchMarker.alien_up then
        local velX, velY = self.launchMarker.alien_up.body:getLinearVelocity()
        local posX, posY = self.launchMarker.alien_up.body:getPosition()
        local sumVel = math.abs(velX) + math.abs(velY)
        if posX < 0 or posX > VIRTUAL_WIDTH*2 or (sumVel < 1.5) then
            self.launchMarker.alien_up.body:destroy()
            self.launchMarker.alien_up = nil
        end
    end
    if self.launchMarker.alien_down then
        local velX, velY = self.launchMarker.alien_down.body:getLinearVelocity()
        local posX, posY = self.launchMarker.alien_down.body:getPosition()
        local sumVel = math.abs(velX) + math.abs(velY)
        if posX < 0 or posX > VIRTUAL_WIDTH*2 or (sumVel < 1.5) then
            self.launchMarker.alien_down.body:destroy()
            self.launchMarker.alien_down = nil
        end
    end
...
function Level:render()
    ...
    if DEBUG then
        if self.launchMarker.alien then
            local px,py = self.launchMarker.alien.body:getPosition()
            local vx,vy = self.launchMarker.alien.body:getLinearVelocity()
            local sv = math.round(math.abs(vx)+math.abs(vy))
            displaytext('alien : '..tostring(math.round(px))..', '..tostring(math.round(py))..' ::: speed : '..(tostring(sv)),10,10)
        end
        if self.launchMarker.alien_up then
            local px,py = self.launchMarker.alien_up.body:getPosition()
            local vx,vy = self.launchMarker.alien_up.body:getLinearVelocity()
            local sv = math.round(math.abs(vx)+math.abs(vy))
            displaytext('alien up : '..tostring(math.round(px))..', '..tostring(math.round(py))..' ::: speed : '..(tostring(sv)),10,20)
        end
        if self.launchMarker.alien_down then
            local px,py = self.launchMarker.alien_down.body:getPosition()
            local vx,vy = self.launchMarker.alien_down.body:getLinearVelocity()
            local sv = math.round(math.abs(vx)+math.abs(vy))
            displaytext('alien down : '..tostring(math.round(px))..', '..tostring(math.round(py))..' ::: speed : '..(tostring(sv)),10,30)
        end

        displaytext('triple : '..(tostring(self.launchMarker.allowTripleBird)),10,40)
        displaytext('collision : '..(tostring(self.collisionOccured)),10,50)
    end
end